...
function DiscussionDetailWrapper({ onNavigate, studentName }: { onNavigate: (screen: string) => void; studentName: string; }) {
  const { id } = useParams();
  return <DiscussionDetailScreen postId={(id as string) || ""} onNavigate={onNavigate} studentName={studentName} />;
}

function ActivityDetailWrapper() {
  const { id } = useParams();
  const navigate = useNavigate();
  return <ActivityDetailScreen activityId={(id as string) || ""} onNavigate={(screen) => { if (screen === "activity-history") navigate("/activity-history"); }} />;
}

function FacilityDetailsWrapper() {
  const { id } = useParams();
  const navigate = useNavigate();
  return <FacilityDetailsScreen facilityId={id || ""} onNavigate={(screen, data) => { if (screen === "time-slot" && data) navigate(`/facility/${data}/time-slot`); if (screen === "book") navigate("/book"); }} />;
}

function TimeSlotWrapper() {
  const { id } = useParams();
  const navigate = useNavigate();
  return <TimeSlotSelectionScreen facilityId={id || ""} onNavigate={(screen, data) => { if (screen === "booking-confirmation") navigate(`/booking/confirmation`, { state: data }); if (screen === "facility-details") navigate(`/facility/${id}`); }} />;
}
...
function BookingConfirmationWrapper() {
  ...
  return <BookingConfirmationScreen bookingData={bookingData || { facilityName: "", date: "", time: "" }} onNavigate={(screen, data) => { if (screen === "time-slot") navigate(`/facility/${facilityId}/time-slot`); if (screen === "success") navigate("/booking/success", { state: data }); }} />;
}

function BookingSuccessWrapper() {
  const location = useLocation();
  const navigate = useNavigate();
  const bookingData = (location.state as any) || ({ facilityName: "", date: "", time: "" } as any);
  return <SuccessScreen bookingData={bookingData} onNavigate={(screen) => { if (screen === "upcoming" || screen === "my-bookings") navigate("/my-bookings"); if (screen === "home") navigate("/home"); }} />;
}

function EditActivityWrapper({ userId, userRole }: { userId: string; userRole: "student" | "staff" }) {
  const { id } = useParams();
  const navigate = useNavigate();
  return <EditActivityScreen activityId={id || ""} userId={userId} userRole={userRole} onNavigate={(screen) => { if (screen === "activity-main") navigate("/activity-main"); }} />;
}

function DetailActivityWrapper() {
  const { id } = useParams();
  const navigate = useNavigate();
  return <DetailActivityScreen activityId={id || ""} onNavigate={(screen) => { if (screen === "activity-main") navigate("/activity-main"); }} />;
}


function RequireAuth({ authed, children }: { authed: boolean; children: ReactNode }) {
  return authed ? <div className="w-full flex-1 flex flex-col bg-[--bg-primary)]">{children as any}</div> : <Navigate to="/" replace />;
}
...
export default function App() {
  ...
  const activeTab = useMemo(() => {
    ...
    if (p.startsWith("/activity") || p.startsWith("/detailactivity") || p.startsWith("/badges") || p.startsWith("/activity-main")) return "activity";
  ...
  const onTabChange = (tab: string) => {
    const map: Record<string, string> = { home: "/home", discussion: "/discussion", book: "/book", activity: "/activity-main", profile: "/profile" };
      ...
      <div className="h-full w-full bg-[--bg-primary)] text-[--text-primary)] transition-colors duration-300 flex flex-col overflow-hidden">
        {hasHeader && (
          <header className="fixed-header-top">
            ...
            {location.pathname.match(/^\/activity\/[^/]+$/) && !location.pathname.startsWith("/activity/record") && <ActivityDetailHeader onBack={() => navigate("/activity-history")} />}
          </header>
        )}

        <main className={`content flex-1 overflow-y-auto bg-[--bg-primary)] ${showBottomNav ? "with-bottom-nav" : ""}`} style={{ paddingTop: hasHeader ? "70px" : "0px", minHeight: "100%" }}>
